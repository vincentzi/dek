...
def get_hadoop_spark_session(
    app_name='triangletask', spark_config: SparkConfig = DEFAULT_SPARK_CONFIG
) -> SparkSession:
    ...
    spark_conf.set(
        "spark.dynamicAllocation.minExecutors", spark_config.min_executors
    )
    ...
    spark_conf.set(
        "spark.sql.shuffle.partitions", spark_config.shuffle_partitions
    )
...
class SparkMixin(LogMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if spark:
            self.logger.info(
                f'Using an existing SparkSession {reprlib.repr(spark)}.'
            )
            self.spark = spark
        else:
            ...
            self.logger.info(
                f'Created a new SparkSession {reprlib.repr(self._spark)}.'
            )
    ...
    def _patch_transform(self):
        ...
        def transform(
            input_df: DataFrame, f: Callable[[DataFrame], DataFrame], **kwargs
        ):
    ...
    def _patch_withCustomColumn():
        ...
        def withCustomColumn(
            input_df: DataFrame,
            column_name: str,
            f: Callable[[DataFrame, str], DataFrame],
        ):
    ...
    @staticmethod
    def map_cols(
        column_names: Iterable[str], alias: str = None
    ) -> List[Column]:
        if alias:
            return [
                col(f'{alias}.{column_name}') for column_name in column_names
            ]
        else:
